...
Pipeline (exact match to sam-img2img__1_.ipynb / evaluation_pipeline.ipynb):
  ...
  Step 1 – ControlNet generation
    ...
    steps=25  |  guidance=7.0  |  controlnet_scale=1.5

  Step 2 – SAM + colour assignment + blend
    SAM model:   vit_b  (sam_vit_b_01ec64.pth)
    Outer mask:  SamPredictor — 1 fg point (sketch centroid) + 4 bg corner points
                 → binary_closing(5×5) + binary_fill_holes
  ...
  Step 3 – ControlNet img2img
    ...
    strength=0.50  |  guidance=7.0  |  controlnet_scale=1.5  |  steps=25
...
SAM_MIN_MASK_AREA:     int   = 800

PROMPT = (
    "a single clothing item, centered, studio product photo, "
    "light grey background, no model, no mannequin, high quality"
)
NEGATIVE_PROMPT = (
    "person, model, mannequin, hanger, rack, room, floor, wall, props, "
    "accessories, text, logo, watermark, extra garment, multiple garments, "
    "clutter, colored background, pattern"
)
...
def _get_outer_mask(
    ...
    """
    Prompted SAM: 1 foreground point + 4 corner background points.
    Returns bool (H,W). Matches `prompted_outer_mask` in both notebooks.
    """
    H, W = image_np.shape[:2]
    predictor.set_image(image_np)

    if sketch_gray is not None:
        fg = _sketch_foreground_point(sketch_gray)
    else:
        ...
        fg = (
            np.array([int(np.median(xs)), int(np.median(ys))])
            if len(xs) else np.array([W // 2, H // 2])
        )

    point_coords = np.array(
        [fg, [5, 5], [W - 6, 5], [5, H - 6], [W - 6, H - 6]],
        dtype=np.float32,
    )
    ...
    best  = (
        np.where(valid)[0][np.argmax(scores[valid])]
        if valid.any()
...
@torch.no_grad()
def _run_controlnet(sketch_pil: Image.Image, seed: Optional[int]) -> Image.Image:
...
def generate_image_controlnet(
    ...
    cn_output = _run_controlnet(sketch_pil, seed)
    ...
    final = _run_controlnet_img2img(blended_pil, sketch_pil, seed, img2img_strength)
